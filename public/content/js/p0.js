...
$.extend({ //extend the jquery framework with new screen object
    shiftScreen: new function () { //this object encapsulates the p0 behavior
        ...
        _self.initialize = function (container) {
            ...
            var _transcisiones=function(){
            $('#title', _container).animate({'margin-left':'20px','-moz-transition': '500s;'});
            $('#a1', _container).animate({'margin-top':'20px','-moz-transition': '400s;'});
            $('#a2', _container).animate({'margin-top':'20px','-moz-transition': '300s;'});
            $('#bloqueColor', _container).animate({'margin-top':'20px','-moz-transition': '200s;'});
        ...
        var _attachBehavior = function () {            
            ...
            $('button.message', _container).click(function(){
                MessageBox.alert('Mensaje de alerta', 'Me adueño del viento y también se los brindo porque a mí ya no me sirve: Solo me trae el recuerdo de quien fue y ahora no es. Todo el susurra la voz que ya no oigo y dibuja el rostro que ya no veo.');
            });
